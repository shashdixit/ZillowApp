...
async def process_borrower_names(filename, session, system_prompt, message_prompt, all_column_names, PDF_DIRECTORY):
    """Process data for the borrower names table."""
    pdf_path = os.path.join(PDF_DIRECTORY, filename)
    print(f"Processing Borrower Names Table for {filename}...")
    ...
    try:
        ...
        rnames = parsed_results.get("raw_borrower_individual_full_name", "").split(",")
        names = parsed_results.get("borrower_individual_full_name", "").split(",")
        rni_names = parsed_results.get("raw_borrower_non_individual_name", "").split(";")
        ni_names = parsed_results.get("borrower_non_individual_name", "").split(";")
        ...
        while i + j < num_names:
            ...
            if i < n:
                output_row.append("")
                output_row.append(" ".join(names[i].strip().split()[:-1]))  # borrower_first_middle_name
                output_row.append("")
                output_row.append(names[i].strip().split()[-1])  # borrower_last_name
                if i < rn:
                    output_row.append(rnames[i].strip())  # raw_borrower_individual_full_name
                else:
                    output_row.append("")
                output_row.append(names[i].strip())  # borrower_individual_full_name
            else:
                output_row.extend(["", "", "", "", "", ""])  # Empty values if no individual name


            if i == n and j < rni:
                output_row.append(rni_names[j].strip())  # raw_borrower_non_individual_name
                flag = 0
            else:
                output_row.append("")

            if i == n and j < ni:
                output_row.append(ni_names[j].strip())  # borrower_non_individual_name
            else:
                output_row.append("")

            output_row.append(i+j+1)  # borrower_name_sequence_number
            output_row.append(1)  # borrower_mail_sequence_number
        ...
        print(f"Error processing Borrower Names Table for {filename}: {e}")
        return None